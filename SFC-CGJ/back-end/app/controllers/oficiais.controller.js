...
const Oficial = require("../models/oficial.model.js");



// Retrieve all Oficial from the database.
exports.findAll = (req, res) => {
  Oficial.getAll((err, data) => {
      if (err)
        res.status(500).send({
          message:
            err.message || "Some error occurred while retrieving Oficiais."
        });
      else res.send(data);
    });
  };
...
// Retrieve only the cpf and name of all Oficial from the database.
exports.findAllCpfNome = (req, res) => {
  Oficial.getAllCpfNome((err, data) => {
      if (err)
        res.status(500).send({
          message:
            err.message || "Some error occurred while retrieving Oficiais."
